...
# Importamos las funciones de filtrado del script clean_ecg_scipy 
...
def bmi_and_note(height_cm, weight_kg):
    ...
    # Calcula IMC y categoría OMS si hay datos válidos
    # DevuelveRetorna (texto_valor, texto_categoria) o (None, None) si no procede
    ...
    try:
        ...
        if h <= 0 or w <= 0:
            return None, None
        # Si la altura viene en metros (<=3), usar tal cual; si no, pasar de cm a m
...
def build_patient_context(row):
    ...
    # Extrae patient_id, sex, age, height, weight del Excel y prepara:
    ...
    # Grupo de edad + recomendaciones CV
...
def build_llm_prompt(signal_name: str, fs: float, pred_name: str, probs_ranked, class_names, patient_ctx_text: str):
...
# --------------------------- Traducción forzada a ES --------------------------
def translate_to_es_with_llm(text: str, model_name: str, backend: str = "ollama") -> str:

    # Traduce usando el propio LLM. Conserva formato, viñetas y símbolos.
...
Objetivo: devuelve EXCLUSIVAMENTE el texto traducido al castellano (es-ES), sin explicaciones ni notas adicionales.
Requisitos:
- Conserva saltos de línea, sangrías, viñetas y símbolos.
- No resumas, no reescribas estilo, no añadas nada.
...
""".strip()
    ...
    if backend == "ollama":
        return call_ollama_llama(model_name=model_name, prompt=tr_prompt)
    else:
        # Si usas llama_cpp, aquí tendrías que implementar su llamada.
        return text

# PDF 

def save_report_pdf(text_body: str, out_path: str, title: str, patient_block: str):
    # Reemplaza caracteres fuera de latin-1 que suelen colarse desde LLM/Word
    def sanitize_text(s: str | None) -> str:
        ...
        return (
            s.replace("\u2014", "-")   # — em-dash
             .replace("\u2013", "-")   # – en-dash
             .replace("\u2212", "-")   # − minus matemático
             .replace("\u2022", "- ")  # • bullet
             .replace("•", "- ")       # bullet directo
...
def main(args):
    ...
    # Ruta WFDB (sin extensión), relativa a base_dir
    ...
    if args.make_report:
        try:
            ...
            if args.llm_backend == "ollama":
                ...
                raise ValueError("Para llama_cpp, implementa la llamada y pasa --llm_model con ruta al .gguf")

            # Forzar traducción a castellano 
           
            report_text = translate_to_es_with_llm(report_text, "llama3:8b", backend=args.llm_backend)
            ...
            # Guardar PDF con título y bloque del paciente
            save_report_pdf(
                text_body=report_text,
                out_path=str(out),
                title=title_text,
                patient_block=patient_ctx["pdf_text"],
            )
            ...
            print("       ¿Está el servidor arrancado?  ->  ollama serve   y   ollama run hf.co/mradermacher/ClinicalGPT-Pubmed-Instruct-V1.0-GGUF")
            print("Detalle:", e)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--checkpoint",
        type=str,
        default="C:/.../0_ptb-xl-a-large-publicly-available-electrocardiography-dataset-1.0.3/zzz_results_inception1d/inception1d_best_foldfixed.pt",
        help="Ruta al checkpoint (.pt)",
    )
    parser.add_argument(
        "--excel_path",
        type=str,
        default="C:/.../0_ptb-xl-a-large-publicly-available-electrocardiography-dataset-1.0.3/ptbxl_database_test.xlsx",
    ...
    parser.add_argument(
        ...
        type=str,
        default="inception1d",
        help="Modelo del checkpoint (resnet1d | inception1d | cnn_baseline | sincnet)",
    )
    parser.add_argument(
        "--base_dir",
        type=str,
    ...
    parser.add_argument(
        "--make_report",
        action="store_true",
        help="Genera un informe con un LLM a partir de la inferencia.",
    )
    parser.add_argument(
        "--llm_backend",
        type=str,
        ...
        help="Backend LLM: 'ollama' (REST local) o 'llama_cpp' (.gguf local).",
    )
    parser.add_argument(
        "--llm_model",
        type=str,
        default="hf.co/mradermacher/ClinicalGPT-Pubmed-Instruct-V1.0-GGUF",
    ...
    parser.add_argument(
        ...
        help="Ruta de salida del informe PDF.",
    )

...
def main(args):
    ...
    print("Comenzando el proceso de inferencia para obtener superclase (backend Mistral)")
    ...
    if args.make_report:
        try:
            ...
            if args.llm_backend == "ollama":
                report_text = call_ollama_mistral(model_name=args.llm_model, prompt=prompt)
            else:
                raise ValueError("Backend no soportado: usa --llm_backend ollama")
            ...
            print("       ¿Está el servidor arrancado?  ->  ollama serve   y   ollama pull mistral:7b")
            print("Detalle:", e)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Inferencia PTB-XL (superclase) con Mistral (vía Ollama)")
    ...
    parser.add_argument(
        ...
        default="ollama",
        choices=["ollama"],
        help="Backend LLM: por ahora soportado 'ollama' (REST local).",
    )
    parser.add_argument(
        "--llm_model",
        type=str,
        default="mistral:7b",  
        help="Nombre del modelo en Ollama (p.ej., mistral:7b, mixtral:8x7b).",
    )
    parser.add_argument(
        "--report_path",
        type=str,
        default="C:/.../informe_inferencia_ecg_mistral.pdf",
